...
class Home_model extends MY_Model {
    ...
    function getPackageByDestination($condition_array = array(),$country_id = null) {
            ...
            $this->db->join('countries c', 'c.id = p.country_id','inner');
    ...
    function getFeaturedDestination() {
        ...
        $this->db->distinct();
        $this->db->select('c.id,c.name,c.weightage,c.slug');
        $this->db->from('countries c');
        $this->db->join('tour_package p', 'c.id = p.country_id','inner');
        $this->db->where('c.show_home', 1);
        $this->db->order_by('c.weightage', 'ASC');
        $this->db->limit(6);
