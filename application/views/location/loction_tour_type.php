    ...
    <div id="content">
        <div class="m-b-10">
            ...
            <div class="pull-right">
                <a href="<?php echo base_url('loction/addLoctionTourType'); ?>" class="btn btn-primary">
                    <i class="fa fa-plus"></i>Add Location Tour Type
                </a>
            </div>
        </div>
                            ...
                            <div class="widget-body no-padding">
                                <div id="dt_basic_wrapper" class="dataTables_wrapper form-inline no-footer">
                                    <table width="100%" class="table table-striped table-bordered table-hover dataTable no-footer" id="dt_basic" role="grid" aria-describedby="dt_basic_info" style="width: 100%;">
                                        ...
                                        <tbody>
                                            <?php
                                            if (!empty($loction_tour_type_list)) {
//                                                dump($loction_tour_type_list);die;
                                                foreach ($loction_tour_type_list as $loction_tour_type) {
                                                    ?>
                                                    <tr role="row" class="even">
                                                        <td class=" expand">
                                                            <?php echo !empty($loction_tour_type->loction) ? $loction_tour_type->loction->loction : '' ?>
                                                        </td>
                                                        
                                                        <td class=" expand">
                                                            <?php echo!empty($loction_tour_type->country) ? $loction_tour_type->country->name : '' ?>
                                                        </td>
                                                        <td class=" expand">
                                                            <?php echo !empty($loction_tour_type->type) && ($loction_tour_type->type == 1) ? 'Tour Type' : 'Activities' ?>
                                                        </td>
                                                        <td>
                                                            <a href="<?php echo base_url('loction/editTourTypeLoction/'.$loction_tour_type->id); ?>" class="btn btn-primary">Edit</a>
                                                            <a href="" class="delete btn btn-sm btn-default" data-record-id="<?php echo $loction_tour_type->id ?>" data-record-title="" data-toggle="modal" data-target="#confirm-delete">
                                                                Delete
                                                            </a>
                                                        </td>
                                                    </tr>
